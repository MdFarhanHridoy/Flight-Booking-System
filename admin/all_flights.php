...
<?php
if (isset($error) && $error === "fkerror") {
    echo "<script>alert('Flight is booked');</script>";
}
?>
<?php
if (isset($_POST['update_status']) and isset($_SESSION['adminId'])) {
    $flight_id = $_POST['flight_id'];
    $new_status = $_POST['flight_status'];
    
    $update_query = "UPDATE Flight SET status = ? WHERE flight_id = ?";
    ...
    if (!mysqli_stmt_prepare($update_stmt, $update_query)) {
      ...
      mysqli_stmt_bind_param($update_stmt, "si", $new_status, $flight_id);
